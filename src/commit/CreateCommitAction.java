...
public class CreateCommitAction extends AnAction implements DumbAware {
    ...
    public void actionPerformed(AnActionEvent actionEvent) {
        ...
        CommitMessage commitMessage = parseExistingCommitMessage(commitPanel,actionEvent.getProject());
        CommitDialog dialog = new CommitDialog(actionEvent.getProject(), commitMessage);
        dialog.show();

        if (dialog.getExitCode() == DialogWrapper.OK_EXIT_CODE) {
            commitPanel.setCommitMessage(dialog.getCommitMessage(actionEvent.getProject()).toString());
        }
    }

    private CommitMessage parseExistingCommitMessage(CommitMessageI commitPanel,  Project project) {
